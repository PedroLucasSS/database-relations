...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (!customerExists) {
      throw new AppError('Could not find any costumer with this id');
    }
    ...
    const checkInexixtentProducts = products.filter(
      product => !productsExistsIds.includes(product.id),
    );

    if (checkInexixtentProducts.length) {
      throw new AppError(
        `Could not find product ${checkInexixtentProducts[0].id}`,
      );
    }
